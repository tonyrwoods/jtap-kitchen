...
async function handleOrderApproved(base44: any, eventData: any): Promise<Response> {
  ...
  // Event center deposit: mark the inquiry's deposit paid and email the guest.
  // Idempotent: only act while the deposit is still "Unpaid".
  if (purchase.productId && purchase.productId.startsWith("eventdeposit:")) {
    const inquiryId = purchase.productId.slice("eventdeposit:".length);
    try {
      const inquiry = await db.entities.EventCenterInquiry.get(inquiryId);
      if (!inquiry) {
        throw new Error("Event inquiry not found for paid deposit");
      }
      if (inquiry.deposit_status === "Unpaid") {
        await db.entities.EventCenterInquiry.update(inquiryId, {
          deposit_status: "Paid",
          status: "New",
          stripe_payment_id: orderId ?? inquiry.stripe_payment_id ?? null,
        });
        try {
          await sendEventDepositConfirmationEmail(base44, inquiry, purchase);
        } catch (mailErr) {
          console.error("payments-webhook: event deposit confirmation email failed", mailErr);
        }
      }
    } catch (e) {
      console.error("payments-webhook: event deposit fulfillment failed", e);
      throw e;
    }
  }
...
async function sendEventDepositConfirmationEmail(base44: any, inquiry: any, purchase: any): Promise<void> {
  const refNo = (inquiry.id || "").substring(0, 8).toUpperCase();
  const deposit = Number(purchase?.amount ?? inquiry.deposit_amount ?? 0);
