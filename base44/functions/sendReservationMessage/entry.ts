...
// Per-reservation ad-hoc email — admin types a custom subject + message to a guest.
...
<html><body style="font-family:Georgia,serif;background:#faf9f7;padding:40px 20px;color:#1a1a1a;margin:0;">
  <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #e8e0d5;">
    ...
    <div style="padding:40px 36px;">
      <h2 style="font-size:22px;margin:0 0 8px;">Hi ${esc(firstName)},</h2>
      <div style="background:#f5f3f0;border-radius:12px;padding:16px 20px;margin:0 0 24px;">
        <p style="margin:0 0 6px;font-size:13px;"><strong>Reservation:</strong> ${formattedDate} at ${esc(reservation.time)}</p>
    ...
    console.error('sendReservationMessage error:', error);
    await notifyAdmins(base44, {
      subject: 'Per-reservation guest email failed',
      body: `An admin email to a reservation guest could not be sent.<br><br><strong>Error:</strong> ${error.message}<br><strong>Time:</strong> ${new Date().toISOString()}`,
    }).catch(() => {});
