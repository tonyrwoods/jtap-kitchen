...
export default async function (req) {
  try {
    ...
    const { reservation_id, message } = await req.json().catch(() => ({}));
    if (!reservation_id) return Response.json({ error: 'reservation_id is required' }, { status: 400 });
    if (!message || !message.trim()) return Response.json({ error: 'A reply message is required' }, { status: 400 });
    ...
    const subject = `Re: Your Reservation Request — ${formattedDate}`;
...
<html><body style="font-family:Georgia,serif;background:#faf9f7;padding:40px 20px;color:#1a1a1a;margin:0;">
  <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #e8e0d5;">
    <div style="background:#1a1a1a;padding:32px;text-align:center;">
      ...
      <p style="color:#999;font-size:13px;margin:8px 0 0;letter-spacing:2px;text-transform:uppercase;">A Note From Our Team</p>
    </div>
    <div style="padding:40px 36px;">
      <h2 style="font-size:22px;margin:0 0 8px;">Hi ${esc(reservation.guest_name)},</h2>
      <p style="color:#555;line-height:1.7;margin:0 0 24px;">Thank you for sharing your request with us. Here is our reply regarding your upcoming reservation:</p>
      <div style="background:#f5f3f0;border-radius:12px;padding:20px;margin:0 0 28px;">
        ...
        ${reservation.special_requests ? `<p style="margin:12px 0 0;font-size:13px;color:#777;font-style:italic;"><strong>Your request:</strong> ${esc(reservation.special_requests)}</p>` : ''}
      </div>
      <div style="background:#fff;border-left:4px solid #C89B4F;border-radius:8px;padding:18px 20px;margin:0 0 28px;">
        <p style="margin:0;font-size:15px;line-height:1.7;color:#1a1a1a;white-space:pre-line;">${esc(message.trim())}</p>
      </div>
      <p style="color:#666;font-size:14px;line-height:1.6;margin:0 0 20px;">We look forward to welcoming you. If you have any further questions, simply reply to this email or call us at (901) 213-8085.</p>
    ...
    console.error('sendReservationNoteReply error:', error);
