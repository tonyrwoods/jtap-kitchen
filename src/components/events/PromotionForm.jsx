...
const EMPTY = {
  ...
  rsvp_deadline: "", max_guests: 50, price_per_guest: 0, host_name: "",
...
export default function PromotionForm({ promotion, onSave, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const data = { ...form, max_guests: parseInt(form.max_guests) || 0, price_per_guest: parseFloat(form.price_per_guest) || 0 };
      if (!promotion?.id) {
      ...
      if (promotion?.id) {
        ...
        await base44.entities.EventPromotion.create(data);
      }
      toast.success(promotion?.id ? "Promotion updated" : "Promotion created");
      onSave();
    } catch (err) {
